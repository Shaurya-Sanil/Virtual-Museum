...
def get_chatgpt_response(user_input):
    try:
        ...
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You are the head of research at a virtual museum dedicated to Indian history and culture. As the curator and expert in this field, you oversee critical research in several key areas, including Artifact Preservation, Textile Research, Ancient Coins, Vedic Scripts, and Archaeological Excavations. Your role involves guiding the museum visitors through these areas of research, providing detailed, scholarly responses that reflect your deep expertise in each subject. When answering questions from museum visitors, respond with authority, clarity, and a passion for preserving and understanding history. Use a formal yet approachable tone, and ensure your responses focus on education, offering insights into these rich cultural topics. While using academic terminology, ensure you explain complex concepts in a way that is easy for visitors to understand, helping them engage with the exhibits more deeply."},
...
def send_message():
    ...
    chat_display.insert(tk.END, f"Head of Research: {short_response}\n")
    chat_display.yview(tk.END)
...
def ask_for_more_details(full_response):
    chat_display.insert(
        tk.END, "Head of Research: Would you like me to explain in more detail?\n")
    ...
    def clarify_response():
        chat_display.insert(
            tk.END, f"Head of Research (Detailed): {full_response}\n")
...
def navigate_to(floor_name):
    ...
    chat_display.insert(tk.END, f"Head of Research: {message}\n")
    play_tts(message)
    chat_display.yview(tk.END)
...
root = tk.Tk()
root.title("Research Floor - Virtual Museum")
...
header_label = tk.Label(header_frame, text="Research Floor - Virtual Museum",
                        bg="#6C4A35", fg="#FFFFFF", font=("Georgia", 18))
header_label.pack(pady=10)

# Title and instructions
title_label = tk.Label(root, text="Welcome to the Research Floor",
                       bg="#FFF8E8", fg="#4A3622", font=("Georgia", 24, "bold"))
title_label.pack(pady=10)

instructions_label = tk.Label(root, text="Explore ongoing research on ancient Indian artifacts and discoveries. Ask your research guide for details.",
                              bg="#FFF8E8", fg="#4A3622", font=("Georgia", 14))
instructions_label.pack(pady=10)

# Buttons for various research sections
button_frame = tk.Frame(root, bg="#FFF8E8")
button_frame.pack(pady=20)

sections = ["Artifact Preservation", "Textile Research",
            "Ancient Coins", "Vedic Scripts", "Archaeological Excavations"]
for section in sections:
    section_button = tk.Button(button_frame, text=section, font=("Georgia", 12), bg="#6C4A35", fg="#FFFFFF",
                               command=lambda t=section: navigate_to(t))
    section_button.pack(side=tk.LEFT, padx=10)
...
chat_display.insert(
    tk.END, "Research Guide: Welcome! I am here to guide you through our ongoing research on ancient Indian artifacts.\n")
